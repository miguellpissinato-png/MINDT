...
var FLORESTA_VB_W = 1200, FLORESTA_VB_H = 560;

// A paisagem e desenhada numa faixa de 460 unidades, como sempre foi. As
// 100 unidades a mais da caixa sao TERRA DE SOBRA embaixo, fora da tela.
//
// Elas existem por causa do teto de rolagem: quando o cenario sobe os seus
// 81px, a borda de baixo do desenho subia junto e aparecia uma faixa de
// fundo vazio no pe da tela. Com a sobra, a colina continua pintada bem
// abaixo do que se ve, e a borda nunca chega a aparecer. O CSS desloca a
// caixa para baixo exatamente nessas 100 unidades, entao o enquadramento
// na tela e o mesmo de antes.
var FLORESTA_VB_SOBRA = 100;
...
var FLORESTA_COLINA_MEIO =
  'M-60 360C90 316 176 336 268 352C382 372 470 330 596 338C712 346 800 320 906 330C1010 340 1120 366 1260 350L1260 560L-60 560Z';
var FLORESTA_COLINA_PERTO =
  'M-60 424C120 400 240 416 392 428C548 440 664 414 810 420C946 426 1080 442 1260 424L1260 560L-60 560Z';
...
// referencia (data-plano = 1). Os outros planos multiplicam por sua
// profundidade — o mais proximo, 1.35, chega a 81px.
//
// ESTE TETO E O CONSERTO.
//
// Antes o deslocamento era `rolagem * 0.34 * profundidade`, sem limite
// nenhum. Duas coisas quebravam por causa disso, e as duas eram visiveis:
//
//   • As arvores da frente andavam 0,46px por pixel rolado. Em ~900px de
//     pagina ja tinham subido 414px e batiam na borda de cima da caixa do
//     desenho, que fica no meio da tela — apareciam cortadas em linha reta.
//   • O chao (0,34px/px) saia junto, e sobravam so as montanhas ao fundo,
//     que a 0,075px/px quase nao se moviam. Dai "o chao some e so ficam as
//     montanhas".
//
// Com teto, o cenario continua reagindo a rolagem exatamente como antes
// nos primeiros pixels — a conta abaixo comeca com a MESMA inclinacao de
// 0,34 — e depois estaciona, em vez de continuar subindo para sempre. A
// paisagem fica inteira na tela por mais longa que a pagina seja.
var FLORESTA_TETO = 60, FLORESTA_TAXA = 0.34;

// Comeca em 0,34px por pixel rolado e se aproxima do teto sem nunca passar
// dele. Sem degrau: a curva e continua e sempre crescente.
function florestaDeslocamento(rolagem){
  if (rolagem <= 0) return 0;
